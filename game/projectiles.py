...
def update_projectiles(game):
    ...
    game["bullets"] = [bullet for bullet in game["bullets"] if bullet[0] < WIDTH + 40]

    for enemy_bullet in game["enemy_bullets"]:
        enemy_bullet[0] -= ENEMY_BULLET_SPEED

    game["enemy_bullets"] = [
        enemy_bullet for enemy_bullet in game["enemy_bullets"] if enemy_bullet[0] > -20
    ]
...
def draw_projectiles(screen, game):
    ...
    for enemy_bullet in game["enemy_bullets"]:
        pygame.draw.rect(
            screen, (255, 80, 80), (enemy_bullet[0], enemy_bullet[1] - 3, 12, 6)
        )
